...
use App\Modules\Member\Requests\MemberAvatarRequest;
...
class MemberAvatarController
{
    ...
    public function list(): JsonResponse
    {
        ...
        $user = Auth::user();
        $user->load(['memberAvatars']);

        $avatarsCount = $user->memberAvatars->count();

        $response = [
            'message' => 'Member has '.$avatarsCount.' avatar files.',
            'total' => $avatarsCount,
            'result' => $user->memberAvatars?->toArray() ?? 0,
        ];
    ...
    public function select(int $avatar_uid): JsonResponse
    {
        ...
        $exists = false;
        foreach ($user->memberAvatars as $avatar) {
    ...
    public function selected(): JsonResponse
    {
        /** @var \App\Modules\User\Models\User $user */
        $user = Auth::user();
        $user->load(['memberAvatar']);

        $avatar = $user->memberAvatar;
        if (! $avatar) {
            return response()->json([
    ...
    public function upload(Request $request): JsonResponse
    {
        ...
        $formRequest = new MemberAvatarRequest;
        ...
        $max = 10;
        $total = MemberAvatar::query()
            ->where('user_id', $user->id)
            ->count();
        if ($total >= $max) {
            return response()->json(
                [
                    'message' => 'Maximum avatars upload per member is '.$max.'.',
        ...
        $avatar->slug = Str::slug($user->memberProfile->nickname);
